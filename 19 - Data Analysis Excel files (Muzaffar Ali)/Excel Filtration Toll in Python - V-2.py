...
# Name:                 Excel Filtration Toll in Python - V-2
...
def search_and_save_excel(input_file, search_column_index, search_value, output_file):
    ...
    # Convert the column to string and check if it contains the search value
    search_results = df[df[search_column].astype(str).str.contains(str(search_value))]
...
# Define the input and output file paths, column index to search, and the search value
input_file = 'Searching_Results.xlsx'
search_column_index = 8
search_value = 'XXL'
